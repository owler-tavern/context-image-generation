...
import { capabilityIsSupported, normalizeModelCapabilities, normalizeModelDefinition } from './providers/contracts.js';
...
export function resolveEffectiveImageSize(savedImageSize, capabilities = {}) {
    const saved = typeof savedImageSize === 'string' ? savedImageSize : '';
    const normalized = normalizeModelCapabilities(capabilities);
    return capabilityIsSupported(normalized.sizes)
        && Array.isArray(normalized.allowedSizes)
        && normalized.allowedSizes.includes(saved)
        ? saved
        : '';
}
...
export function createGenerationPlan(input = {}) {
    ...
    const effectiveImageSize = resolveEffectiveImageSize(input.options?.imageSize, provider.capabilities);
    ...
    const plan = {
        ...
        referenceOmissions: [...referenceSelection.omitted, ...(Array.isArray(input.referenceOmissions) ? clone(input.referenceOmissions) : [])],
        options: {
