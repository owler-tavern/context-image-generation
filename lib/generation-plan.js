...
import { applyDirectorCastToReferences, validateDirectorCastOverrides } from './rp/director-cast.js';
...
function copySceneSnapshot(scene) {
    ...
    const inspection = scene.inspection && typeof scene.inspection === 'object' && !Array.isArray(scene.inspection) ? clone(scene.inspection) : null;
    if (inspection && Array.isArray(scene.castOverrides) && scene.castOverrides.length) inspection.castOverrides = clone(scene.castOverrides);
    return {
        ...
        inspection,
        ...(Array.isArray(scene.castOverrides) ? { castOverrides: clone(scene.castOverrides) } : {}),
    };
}
...
export function createGenerationPlan(input = {}) {
    ...
    const providedReferences = (Array.isArray(input.references) ? clone(input.references) : [])
        .filter((reference) => !availableReferenceIds || availableReferenceIds.has(String(reference.id)));
    const baseReferences = [...providedReferences, ...appearanceReferences]
        .filter((reference, index, all) => all.findIndex((candidate) => candidate.id === reference.id) === index);
    const castOverrides = Array.isArray(input.options?.castOverrides) ? input.options.castOverrides : input.scene?.castOverrides;
    const castValidation = validateDirectorCastOverrides(castOverrides, {
        identities: input.identities,
        cast: input.scene?.state?.sceneFacts?.cast,
    });
    if (!castValidation.valid) throw new TypeError(`Director cast override identity is not valid for this message: ${castValidation.invalidIdentityIds.join(', ')}`);
    const castReferencePolicy = applyDirectorCastToReferences({ references: baseReferences, overrides: castValidation.overrides });
    const references = castReferencePolicy.references;
    let referenceSelection = input.referencePlan && Array.isArray(input.referencePlan.selected) && Array.isArray(input.referencePlan.omitted)
        ? (() => {
            const selectedIds = new Set(input.referencePlan.selected.map((reference) => String(reference?.id || '')));
            const forcedIds = new Set(castReferencePolicy.forcedIdentityIds);
            const omittedById = new Map(input.referencePlan.omitted.map((entry) => [String(entry?.id || ''), entry]));
            const ordered = references.filter((reference) => forcedIds.has(String(reference.identityId || '')) || selectedIds.has(String(reference.id)));
            const maxCount = provider.capabilities?.referenceImages?.maxCount;
            const selected = Number.isInteger(maxCount) && maxCount >= 0 ? ordered.slice(0, maxCount) : ordered;
            const selectedReferenceIds = new Set(selected.map((reference) => String(reference.id)));
            const omitted = references
                .filter((reference) => !selectedReferenceIds.has(String(reference.id)))
        ...
        : selectReferenceCandidates(
            references,
            {
                ...
                forceIdentityIds: castReferencePolicy.forcedIdentityIds,
    ...
    referenceSelection = {
        ...referenceSelection,
        omitted: [
            ...castReferencePolicy.omitted.map(({ reason, ...candidate }) => ({ candidate, reason })),
    ...
    const plan = {
        ...
        referenceOmissions: [...castReferencePolicy.omitted, ...referenceSelection.omitted, ...(Array.isArray(canonSnapshot?.omissions) ? canonSnapshot.omissions : []), ...(Array.isArray(input.referenceOmissions) ? clone(input.referenceOmissions) : [])],
        options: {
            ...
            ...(castValidation.overrides.length ? { castOverrides: clone(castValidation.overrides) } : {}),
        },
        policy: {
