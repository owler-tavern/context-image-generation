export function createOptionalFeatureLifecycle({ load, setup = () => undefined, teardown = () => undefined, onError = () => undefined } = {}) {
    ...
    async function enable() {
        ...
        while (enabled && generation === capturedGeneration) {
            if (pending) {
                if (pendingGeneration === capturedGeneration) return pending;
                await pending;
                continue;
            }
            if (cleanup) {
            ...
            if (resource) {
                if (resourceGeneration === capturedGeneration) return { status: 'ready' };
                await release(resource, resourceGeneration);
                continue;
            }
