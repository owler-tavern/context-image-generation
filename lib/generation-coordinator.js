...
function deadlineError() {
    const error = new Error('Image generation timed out before it could finish.');
    error.name = 'GenerationTimeoutError';
    error.category = 'timeout';
    error.code = 'GENERATION_TIMEOUT';
    error.userMessage = 'Image generation timed out. Try again.';
    return error;
}

function deadlineMs(value, fallback) {
    const candidate = Number(value ?? fallback);
    return Number.isFinite(candidate) && candidate >= 1 && candidate <= 15 * 60 * 1000 ? candidate : fallback;
}
...
function snapshot(record) {
    ...
    const { controller, resolve, reject, promise, execute, settled, cancelRequested, result, deadlineMs, deadlineTimer, telemetry, callerTerminalized, operationReleased, ...publicRecord } = record;
...
export function createRunCoordinator({ maxConcurrent = 2, maxTerminalRecords = 64, generationDeadlineMs = 120000 } = {}) {
    ...
    generationDeadlineMs = deadlineMs(generationDeadlineMs, 120000);
    ...
    function terminal(record, state, result, error) {
        ...
        record.telemetry.record('coordinate', error?.code === 'GENERATION_TIMEOUT'
            ? 'timed_out'
            : state === 'cancelled' ? 'cancelled' : state === 'failed' ? 'failed' : 'completed');
    ...
    async function start(record) {
        ...
        record.deadlineTimer = setTimeout(() => {
            if (record.callerTerminalized || record.commitStarted) return;
            record.controller.abort();
            terminal(record, 'failed', undefined, deadlineError());
        }, record.deadlineMs);
        try {
    ...
    function enqueue(plan, execute, options = {}) {
        ...
        const record = {
            ...
            deadlineMs: deadlineMs(options.deadlineMs, generationDeadlineMs),
            deadlineTimer: null,
            telemetry: null,
            plan,
            execute,
