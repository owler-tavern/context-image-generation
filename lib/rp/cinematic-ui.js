...
export function renderCinematicSuggestionCard(suggestion) {
    ...
    return `<section class="cig_cinematic_suggestion" data-cig-cinematic-card="${id}" aria-label="Cinematic image suggestion"><strong>Cinematic suggestion</strong><p class="cig_cinematic_suggestion_why">${why}</p><p class="cig_cinematic_suggestion_shot"><strong>Proposed shot:</strong> ${shot}</p><p class="cig_cinematic_suggestion_budget" role="status" aria-live="polite"><strong>Budget:</strong> ${budget}</p><p class="cig_cinematic_suggestion_waiting"><strong>Next:</strong> ${waiting}</p><div class="cig_cinematic_suggestion_adjust"><label for="cig_cinematic_adjust_${id}">Adjust shot</label><input id="cig_cinematic_adjust_${id}" data-cig-cinematic-adjust-input type="text" value="${shot}" maxlength="800" /></div><div class="cig_cinematic_suggestion_actions"><button type="button" class="menu_button" data-cig-cinematic-action="approve" data-cig-cinematic-id="${id}">Approve</button><button type="button" class="menu_button" data-cig-cinematic-action="adjust" data-cig-cinematic-id="${id}">Adjust</button><button type="button" class="menu_button" data-cig-cinematic-action="dismiss" data-cig-cinematic-id="${id}">Dismiss</button></div></section>`;
}
...
export function focusCinematicSuggestionCard({ documentLike = globalThis.document, suggestionId } = {}) {
    ...
    const focusTarget = card.querySelector?.('[data-cig-cinematic-action="approve"]') || card;
...
export function createCinematicUiController({ getSuggestion = () => null, approve, adjust, dismiss, render = () => {} } = {}) {
    async function action(actionName, value) {
        const suggestion = getSuggestion();
        if (!suggestion?.suggestionId) return { status: 'no-suggestion' };
        if (actionName === 'approve' && typeof approve === 'function') return approve(suggestion.suggestionId);
        if (actionName === 'adjust' && typeof adjust === 'function') return adjust(suggestion.suggestionId, { prompt: String(value ?? '').trim() || suggestion.proposedShot });
        if (actionName === 'dismiss' && typeof dismiss === 'function') return dismiss(suggestion.suggestionId);
