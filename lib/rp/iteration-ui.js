...
export function createIterationSurfaceController(options = {}) {
    ...
    async function submit(input = {}) {
        ...
        if (twoUp && state.twoUpAvailable === false) return update({ status: 'error', error: 'Two-up is unavailable because an exact finite cost quote is not available.', message: 'Two-up is unavailable; single-output actions remain available.' });
