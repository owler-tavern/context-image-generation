...
function validateDispatchReceipt(receipt, plan) {
    if (!isRecord(receipt) || !['completed', 'dispatched'].includes(receipt.status) || receipt.planId !== plan.planId || receipt.invocationId !== plan.invocationId || receipt.outputCount !== plan.artifacts.length || !Array.isArray(receipt.artifacts) || receipt.artifacts.length !== plan.artifacts.length) throw new TypeError('Generation receipt is incomplete or does not match the plan and invocation.');
...
export function createIterationSurfaceController(options = {}) {
    ...
    async function quoteAction({ action, twoUp = state.twoUp, changes = {} } = {}) {
        ...
        try {
            ...
            update({ action: normalizedAction, twoUp: Boolean(twoUp), quote, quoteKey: quote ? quoteKey : null, cost: quote?.amount ?? null, message: quote ? `Estimated cost: ${formatCost(quote.amount, quote.currency)} for ${count} image${count === 1 ? '' : 's'}.` : 'Cost unavailable; generation is blocked.' });
            return freeze(result);
        } catch (error) {
            update({ quote: null, quoteKey: null, cost: null, status: 'error', error: error?.message || 'Cost estimate failed.' });
    ...
    async function submit(input = {}) {
        ...
        if (GENERATION_ACTIONS.has(action)) {
            ...
            if (!exactConsent) {
                return update({ status: 'consent-required', quote, cost: quote.amount, consent: { approved: consent.approved === true, outputCount: Number.isInteger(consent.outputCount) ? consent.outputCount : null }, message: `Confirm the exact quote ${formatCost(quote.amount, quote.currency)} for ${count} image${count === 1 ? '' : 's'}.` });
            }
        }
        ...
        const paidConsent = GENERATION_ACTIONS.has(action) ? { approved: true, outputCount: outputCount(twoUp), cost: quote.amount } : undefined;
        ...
        try {
            ...
            const executionPlan = freeze({ ...clone(plan), invocationId });
            if (requestedAction === ACTIONS.MAKE_CANONICAL_ROLES) {
                ...
                const completed = await persistAndReadback(executionPlan, state.originalArtifact, true);
                return update({ plan: executionPlan, artifacts: [clone(state.originalArtifact)], chosenArtifactId: state.originalArtifact.artifactId, originalArtifact: clone(state.originalArtifact), consent: { approved: false, outputCount: null }, ...completed });
            }
            ...
            const completed = await persistAndReadback(executionPlan, artifacts[0], false);
            return update({ ...completed, plan: executionPlan, artifacts, chosenArtifactId: artifacts[0].artifactId, originalArtifact: clone(state.originalArtifact) });
        } catch (error) {
    ...
    async function chooseArtifact(artifactId) {
        ...
        try {
            ...
            for (const discardId of chosenPlan.retention.discardArtifactIds || []) {
                const discarded = await options.discardArtifact({ artifactId: discardId, artifact: clone(state.artifacts.find((item) => item.artifactId === discardId)), originalArtifact: clone(state.originalArtifact), plan: clone(chosenPlan) });
                if (discarded?.status !== 'confirmed' || discarded.artifactId !== discardId || discarded.planId !== chosenPlan.planId || discarded.invocationId !== chosenPlan.invocationId) throw new TypeError('Unchosen image cleanup was not confirmed.');
            }
...
function renderEditor(safe) {
    ...
    if (action === ACTIONS.MAKE_CANONICAL_ROLES) return `<fieldset data-editor-kind="canonical"><legend>Canonical roles</legend>${textControl({ name: 'activeLook.identityId', label: 'Active look identity', value: safe.canonicalRoles?.activeLook?.identityId || '' })}${textControl({ name: 'activeLook.lookId', label: 'Active look', value: safe.canonicalRoles?.activeLook?.lookId || '' })}${textControl({ name: 'priorScene', label: 'Prior scene', value: safe.canonicalRoles?.priorScene || '', multiline: true })}${textControl({ name: 'chatBackground', label: 'Chat background', value: safe.canonicalRoles?.chatBackground || '', multiline: true })}</fieldset>`;
...
export function renderIterationSurface(state = {}) {
    ...
    const buttons = Object.entries(ACTION_LABELS).filter(([action]) => action !== ACTIONS.RETRY_REPAIRED_PROMPT || showRetry).map(([action, label]) => `<button type="button" class="cig-rp-iteration-action" data-iteration-action="${escapeHtml(action)}" style="min-height:44px"${busy ? ' disabled aria-disabled="true"' : ''}>${escapeHtml(label)}</button>`).join('');
    const displayedQuote = safe.quote || (typeof safe.cost === 'number' ? { amount: safe.cost, currency: 'USD' } : null);
    ...
    return `<section class="cig-rp-iteration-surface" data-cig-rp-iteration-surface aria-busy="${busy ? 'true' : 'false'}"><div role="status" aria-live="polite">${escapeHtml(safe.message || '')}</div>${safe.error ? `<div role="alert">${escapeHtml(safe.error)}</div>` : ''}<p>Original retained: <code>${escapeHtml(safe.originalArtifact?.artifactId || 'unknown')}</code></p>${renderEditor(safe)}<fieldset><legend>Generation options</legend><label><input type="checkbox" name="twoUp" style="min-height:44px"${safe.twoUp ? ' checked' : ''} /> Generate two-up</label><label><input type="checkbox" name="paidConsent" style="min-height:44px" /> I approve this estimate</label><div class="cig-rp-cost" aria-live="polite">${cost}</div></fieldset><div class="cig-rp-actions" aria-label="Iteration actions">${buttons}</div>${repair}${chooser}</section>`;
}
...
export function mountIterationSurface(host, controller) {
    ...
    const click = (event) => {
        ...
        const changes = {};
        if (action === ACTIONS.VARY_SHOT) changes.composition = { framing: read('[name="framing"]', ''), camera: read('[name="camera"]', ''), focus: read('[name="focus"]', '') };
        if (action === ACTIONS.KEEP_CHARACTERS_CHANGE_SCENE) changes.sourcePassage = read('[name="sourcePassage"]', '');
        if (action === ACTIONS.KEEP_CHARACTERS_CHANGE_SCENE || action === ACTIONS.EDIT_REGENERATE || action === ACTIONS.RETRY_REPAIRED_PROMPT) changes.prompt = read('[name="prompt"]', current.prompt);
        if (action === ACTIONS.KEEP_CHARACTERS_CHANGE_SCENE) changes.scene = { location: read('[name="scene.location"]', ''), cast: read('[name="scene.cast"]', '') };
        if (action === ACTIONS.MAKE_CANONICAL_ROLES) {
            const identityId = read('[name="activeLook.identityId"]', '');
            const lookId = read('[name="activeLook.lookId"]', '');
            const roles = {};
            if (identityId && lookId) roles.activeLook = { identityId, lookId };
            const priorScene = read('[name="priorScene"]', '');
            const chatBackground = read('[name="chatBackground"]', '');
            if (priorScene) roles.priorScene = priorScene;
            if (chatBackground) roles.chatBackground = chatBackground;
            changes.roles = roles;
        }
        void controller.submit({ action, prompt: read('[name="prompt"]', current.prompt), twoUp, changes, consent }).then(render);
    };
    const edit = (event) => {
        const target = event.target;
        if (!target?.name) return;
        if (target.name === 'prompt') { controller.preparePrompt(target.value); return; }
        const current = controller.getState();
