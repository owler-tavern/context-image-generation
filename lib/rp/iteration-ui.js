...
export function createIterationSurfaceController(options = {}) {
    ...
    function selectAction(action) {
        ...
        return update({ action: normalizedAction, status: 'editing', quote: null, quoteKey: null, message: `${ACTION_LABELS[normalizedAction]} selected. Complete the editor, then submit.`, error: null });
    }
    ...
    async function submit(input = {}) {
        ...
        if (GENERATION_ACTIONS.has(action)) {
            ...
            if (!exactConsent) {
                return update({ status: 'consent-required', quote, consent: { approved: consent.approved === true, outputCount: Number.isInteger(consent.outputCount) ? consent.outputCount : null }, message: `Confirm the exact quote ${formatCost(quote.amount, quote.currency)} for ${count} image${count === 1 ? '' : 's'}.` });
            }
        }
        ...
        try {
            ...
            for (const discardId of chosenPlan.retention.discardArtifactIds || []) {
                ...
                if (discarded?.status !== 'confirmed' || discarded.artifactId !== discardId || discarded.planId !== chosenPlan.planId || discarded.invocationId !== chosenPlan.invocationId) {
                    return update({ status: 'cleanup-pending', plan: chosenPlan, artifacts: clone(state.artifacts), chosenArtifactId: chosen.artifactId, originalArtifact: clone(state.originalArtifact), error: 'Unchosen image cleanup is pending reconciliation.', message: 'The chosen image and original retention are confirmed; cleanup still needs reconciliation.' });
                }
            }
