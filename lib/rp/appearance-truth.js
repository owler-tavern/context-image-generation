...
function avatarMatchesIdentity(avatarId, identityId) {
    return avatarId === identityId || avatarId === identityId.replace(/^(?:character|user|persona|npc):/u, '') || `${identityId.split(':')[0]}:${avatarId}` === identityId;
}
...
function avatarRejection(avatar, identityId) {
    ...
    if (!avatarMatchesIdentity(suppliedIdentityId, identityId)) return { reason: 'different-identity', avatarIdentityId: suppliedIdentityId };
 ...
 */
export function resolveAppearanceTruth({ identity, avatar, description, textTraits = [], sourcePreference: requestedSourceValue = 'auto' } = {}) {
    const identityId = normalizeStableIdentityId(identity?.id);
    ...
    const requestedSource = String(requestedSourceValue || 'auto').trim().toLocaleLowerCase('und');
    const sourcePreference = ['auto', 'avatar', 'description'].includes(requestedSource) ? requestedSource : 'auto';
    ...
    const descriptionText = isRecord(effectiveDescription) ? String(effectiveDescription.text ?? effectiveDescription.description ?? '').trim() : String(effectiveDescription ?? '').trim();
    const avatarAvailable = !rejection && identityId;
    const sourceType = sourcePreference === 'avatar'
        ? (avatarAvailable ? 'avatar' : 'none')
        : sourcePreference === 'description'
            ? (descriptionText ? 'description' : 'none')
            : avatarAvailable ? 'avatar' : descriptionText ? 'description' : 'none';
    if (sourcePreference !== 'auto') evidence.push({ source: 'preference', decision: 'applied', sourceType: sourcePreference });
    if (descriptionText) evidence.push({ source: 'description', decision: sourceType === 'description' ? (sourcePreference === 'description' ? 'selected-explicit' : 'selected-fallback') : 'available-as-fallback' });
    ...
    const structuredDescriptionTraits = isRecord(effectiveDescription) && effectiveDescription.traits != null
        ? (isRecord(effectiveDescription.traits) ? Object.entries(effectiveDescription.traits).map(([key, value]) => ({ key, value, text: value })) : Array.isArray(effectiveDescription.traits) ? effectiveDescription.traits : [effectiveDescription.traits]).map(normalizeTrait).filter(Boolean)
        : [];
    const result = {
        ...
        description: descriptionText ? { text: descriptionText, ...(structuredDescriptionTraits.length ? { traits: structuredDescriptionTraits } : {}) } : null,
