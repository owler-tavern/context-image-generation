...
export function migrateStoryMemory(value = {}, { gallery = [], chatId } = {}) {
    ...
    for (const [id, artifact] of rawArtifacts) {
        const normalizedId = text(artifact?.id || id);
        const normalized = normalizeArtifact({ ...artifact, id: normalizedId }, id);
        const previous = artifacts[normalizedId];
        if (previous) {
            if (previous.url !== normalized.url || stableString(previous.provenance) !== stableString(normalized.provenance)) {
    ...
    for (const artifact of migrateGalleryEntries(gallery, { chatId })) {
        ...
        if (previous) {
            if (previous.url !== artifact.url || stableString(previous.provenance) !== stableString(artifact.provenance)) {
