...
export const STORY_COLLECTION_KINDS = Object.freeze(['canon-look', 'location', 'moment']);
const READABLE_STORY_COLLECTION_KINDS = new Set([...STORY_COLLECTION_KINDS, 'outfit']);
...
function safeReferenceReceipt(value) {
    if (!isRecord(value)) return null;
    if (!Array.isArray(value.used) && !Array.isArray(value.omitted)) return null;
    return compactReferenceReceipt(value);
}
 ...
 */
export function resolveArtifactIdAliasStatus(memoryValue, artifactId) {
    ...
    if (!requested) return { status: 'missing', canonicalId: null };
    ...
    while (current) {
        if (visited.has(current)) return { status: 'cycle', canonicalId: null };
        visited.add(current);
        if (Object.prototype.hasOwnProperty.call(artifacts, current)) {
            const competingTargets = aliasTargets.get(current);
            if (competingTargets && competingTargets.size !== 1) return { status: 'ambiguous', canonicalId: null };
            if (competingTargets?.size === 1 && !competingTargets.has(current)) {
                directConflict = true;
                current = [...competingTargets][0];
                continue;
            }
            return directConflict
                ? { status: 'ambiguous', canonicalId: null }
                : { status: 'resolved', canonicalId: current };
        }
        const targets = aliasTargets.get(current);
        if (!targets) return { status: 'missing', canonicalId: null };
        if (targets.size !== 1) return { status: 'ambiguous', canonicalId: null };
        current = [...targets][0];
    }
    return { status: 'missing', canonicalId: null };
}

export function resolveArtifactIdAlias(memoryValue, artifactId) {
    const resolution = resolveArtifactIdAliasStatus(memoryValue, artifactId);
    return resolution.status === 'resolved' ? resolution.canonicalId : null;
}
...
function normalizeGeneration(source, omittedFields = []) {
    ...
    const referenceReceipt = safeReferenceReceipt(generation.referenceReceipt ?? source.referenceReceipt ?? legacy.referenceReceipt);
    return {
...
function normalizeCollection(value, fallbackId) {
    ...
    const kind = READABLE_STORY_COLLECTION_KINDS.has(source.kind) ? source.kind : 'moment';
...
function mergeArtifactRecords(existing, incoming, id) {
    ...
    return normalizeArtifact({
        ...
        generation: {
            ...
            referenceReceipt: oldGeneration.referenceReceipt || newGeneration.referenceReceipt || null,
        },
...
export function addStoryArtifact(memoryValue, artifactValue) {
    ...
    const candidateId = requestedId || buildStoryArtifactId({ item: source, chatId: source.chatId });
    const resolution = resolveArtifactIdAliasStatus(memory, candidateId);
    if (resolution.status === 'ambiguous' || resolution.status === 'cycle') {
        throw new TypeError(`Cannot resolve story artifact ID: ${candidateId} (${resolution.status}).`);
    }
    let id = resolution.status === 'resolved' ? resolution.canonicalId : candidateId;
    if (memory.artifacts[id] && text(memory.artifacts[id].chatId) !== text(source.chatId) && source.chatId) {
        const fallbackCandidate = buildStoryArtifactId({ item: source, chatId: source.chatId });
        const fallbackResolution = resolveArtifactIdAliasStatus(memory, fallbackCandidate);
        if (fallbackResolution.status === 'ambiguous' || fallbackResolution.status === 'cycle') {
            throw new TypeError(`Cannot resolve story artifact ID: ${fallbackCandidate} (${fallbackResolution.status}).`);
        }
        id = fallbackResolution.status === 'resolved' ? fallbackResolution.canonicalId : fallbackCandidate;
    }
...
export function addStoryCollectionMember(memoryValue, { collectionId, kind = 'moment', label, artifactId } = {}) {
    ...
    const current = memory.collections[id];
    const requestedKind = text(kind) || 'moment';
    if (current?.kind === 'outfit' || (!current && requestedKind === 'outfit')) throw new TypeError('Legacy outfit collections are read-only.');
    const existing = current || normalizeCollection({ id, kind: requestedKind, label: label || id, memberIds: [] }, id);
...
export function createStoryCollection(memoryValue, { collectionId, kind = 'moment', label } = {}) {
    ...
    if (!id) return memory;
    const requestedKind = text(kind) || 'moment';
    if (requestedKind === 'outfit' || memory.collections[id]?.kind === 'outfit') throw new TypeError('Legacy outfit collections are read-only.');
    if (!memory.collections[id]) memory.collections[id] = normalizeCollection({ id, kind: requestedKind, label: label || id, memberIds: [] }, id);
    return memory;
}
...
export function removeStoryCollectionMember(memoryValue, collectionId, artifactId) {
    ...
    const collection = memory.collections[text(collectionId)];
    const artifact = resolveArtifactIdAlias(memory, artifactId);
    if (collection?.kind === 'outfit') throw new TypeError('Legacy outfit collections are read-only.');
