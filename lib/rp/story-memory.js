...
function mergeArtifactRecords(existing, incoming, id) {
    ...
    return normalizeArtifact({
        ...
        aliases: [...new Set([...(existing.aliases || []), ...(incoming.aliases || [])].map(text).filter(Boolean))],
...
export function migrateStoryMemory(value = {}, { gallery = [], chatId } = {}) {
    ...
    for (const [id, artifact] of rawArtifacts) {
        ...
        if (legacyId !== normalizedId) idRemap[legacyId] = normalizedId;
        const normalized = normalizeArtifact({
            ...artifact,
            id: normalizedId,
            aliases: [...new Set([...(artifact?.aliases || []), ...(legacyId !== normalizedId ? [legacyId] : [])].map(text).filter(Boolean))],
        }, id);
    ...
    const collections = Object.fromEntries(rawCollections.map(([id, collection]) => {
        ...
        normalized.memberIds = normalized.memberIds.map((memberId) => idRemap[memberId] || memberId);
