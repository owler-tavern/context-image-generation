...
function operationReason(library, assetId, lookId) {
    const operation = Object.values(library.operations || {}).find((entry) => entry?.assetId === assetId && ['pending', 'deleting'].includes(entry?.status) && (entry.status !== 'deleting' || !(entry.sharedReferenceCount > 0) || entry.lookId === lookId));
...
export function resolveEffectiveLook({ library: libraryValue, chatState, identityId, gallery = [] } = {}) {
    ...
    if (binding) {
        ...
        const guarded = operationReason(library, look.assetId, look.id);
    ...
    if (operationReason(library, look.assetId, look.id)) return { status: 'none', source: 'no-binding' };
    const asset = materializeAppearanceAssets(library, gallery).assets[look.assetId];
