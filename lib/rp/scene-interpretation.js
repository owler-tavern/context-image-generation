...
const CLOTHING_RE = /\b(?:coat|dress|shirt|blouse|jacket|uniform|robe|sweater|pants|trousers|skirt|shorts|clothes|outfit)\b/iu;
...
function extractLocation(parts, index, blockedIds = new Set()) {
    ...
    for (const segment of segments) {
        for (const expression of locationExpressions) {
            ...
            if (value && (!locationLike || KNOWN_PLACE_RE.test(value)) && !CLOTHING_RE.test(value)) {
...
function stateSignal(parts, key, index) {
    const patterns = {
        ...
        outfits: /\b(?:no\s+longer\s+wears?|changes?\s+out\s+of|removes?)\s+(?:a|an|the)?\s*([^,.!?;]+?)(?=\s+(?:and|while|as)\b|[,.!?;]|$)/iu,
    ...
    const matching = usable.filter((segment) => {
        ...
        if (key === 'outfits' && (!match || !CLOTHING_RE.test(match[1] || ''))) return false;
    ...
    if (key === 'location') signal.clear = true;
        ...
        const removalExpression = key === 'outfits'
            ? /\b(?:no\s+longer\s+wears?|changes?\s+out\s+of|removes?)\s+(?:a|an|the)?\s*([^,.!?;]+?)(?=\s+(?:and|while|as)\b|[,.!?;]|$)/iu
            : /\b(?:drops?|discards?|throws?\s+away|sets?\s+down|no\s+longer\s+holds?|isn't\s+holding|aren't\s+holding)\s+(?:a|an|the)?\s*([^,.!?;]+?)(?=\s+(?:and|while|as)\b|[,.!?;]|$)/iu;
        const removal = segment.text.match(removalExpression);
        if (removal?.[1] && (key !== 'outfits' || CLOTHING_RE.test(removal[1]))) {
...
export function interpretScene({ selectedPassage, focusText, clickedMessage, sourceMessage, recentContext, identities: suppliedIdentities, priorStoryState } = {}) {
    ...
    const result = {
        ...
        location,
        outfits: extractDetails(parts, index, 'outfit', new Set(mentionResult.blockedIds)),
        ...
        sceneSignals: {
            ...
            outfits: stateSignal(parts, 'outfits', index),
