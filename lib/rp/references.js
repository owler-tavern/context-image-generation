...
export function rankReferenceCandidates(candidates = [], context = {}) {
    ...
    const forcedIds = new Set(context.forceIdentityIds || []);
    ...
    return [...candidates].sort((left, right) => {
        ...
        const tier = (candidate) => {
            if (candidate.identityId && forcedIds.has(candidate.identityId)) return 6;
...
export function selectReferenceCandidates(candidates = [], context = {}) {
    ...
    if (Number.isInteger(maxCount) && maxCount >= 0) {
        ...
        selected = [];
        omitted = ranked.map((candidate) => ({ candidate, reason: 'unknown-cap' }));
    }
