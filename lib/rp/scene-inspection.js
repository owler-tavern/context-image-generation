...
export function projectSceneInspection(value = {}) {
    ...
    const statuses = Object.fromEntries(['cast', 'location', 'outfits', 'objects', 'injuries'].map((key) => [key, factStatus(value, effective, key, delta)]));
    ...
    const outfits = (effective.outfits || []).map((entry) => `${labelFor(entry.identityId, cast)} — ${entry.value}`).join('; ');
    ...
    return {
        ...
        lines: [
            ...
            `Outfits: ${outfits || (statuses.outfits === 'unknown' ? 'unknown' : 'none recorded')}${suffix(statuses.outfits)}.`,
