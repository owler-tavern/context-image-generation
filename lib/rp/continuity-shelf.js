...
export function alignContinuityReferencePlan(shelfValue, { selected = [], omitted = [] } = {}) {
    ...
    const identities = (Array.isArray(shelf.identities) ? shelf.identities : []).map((identity) => {
        const selectedForIdentity = selectedRows.filter((row) => row.identityId === identity.identityId);
        const omittedForIdentity = omittedRows.filter((row) => row.identityId === identity.identityId);
        const missingVisual = omittedForIdentity.some((row) => row.reason === 'asset-unavailable' || row.reason === 'missing-asset');
        return {
            ...identity,
            selected: selectedForIdentity,
            omitted: omittedForIdentity,
            sourceType: selectedForIdentity[0]?.sourceType || (missingVisual ? 'unavailable' : identity.sourceType),
            ...(selectedForIdentity.length === 0 && missingVisual ? { thumbnail: null } : {}),
        };
    });
    return {
        ...shelf,
...
export function projectContinuityShelf({ identities = [], truths = [], candidates = [], modelLimit, outfitCatalog = {}, outfitState = {}, includeDescriptions = true, includeAvatars = true } = {}) {
    ...
    const visibleCandidates = (Array.isArray(candidates) ? candidates : []).filter((candidate) => (
        (includeDescriptions || candidate?.sourceType !== 'description')
        && (includeAvatars || candidate?.sourceType !== 'avatar')
    ));
    const plan = buildVisibleReferencePlan({ candidates: visibleCandidates, identities: identityList, modelLimit });
    ...
    return {
        ...
        identities: identityList.map((identity) => {
            ...
            const fallbackSource = includeAvatars && truth.sourceType === 'avatar' ? 'avatar' : includeDescriptions && truth.description?.text ? 'description' : 'none';
            return {
                identityId: id,
                identityLabel: identity.label || id,
                sourceType: rows.some((row) => row.status === 'selected') ? sourceRow?.sourceType : fallbackSource,
                thumbnail: sourceRow?.thumbnail || (includeAvatars ? thumbnailForTruth(truth) : null),
                description: includeDescriptions ? truth.description?.text || null : null,
