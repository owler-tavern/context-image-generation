...
export function createCinematicRuntime({
    ...
    const persist = async (state = current) => {
        if (!state) return;
        remember(state);
        const stored = { [CINEMATIC_AUTOMATION_KEY]: clone(state), storyState: clone(state.storyState) };
    ...
    function load({ chatId = getChatId(), epoch = getEpoch() } = {}) {
        ...
        current = chatStates.get(key) || initialState(chatId, settings, readState({ chatId: key }));
        remember(current);
