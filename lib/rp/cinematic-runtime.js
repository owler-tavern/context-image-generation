...
export function createCinematicRuntime({
    ...
    function dismiss(suggestionId) {
        ...
        return enqueue(async () => {
            ...
            if (!source || !currentEnough({ chatId: source.chatId, epoch: getEpoch() }) || text(getChatId()) !== text(source.chatId)) return { status: 'stale', reason: 'The chat changed before this cinematic suggestion could be dismissed.' };
