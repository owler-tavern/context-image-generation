...
function compactSession(session) {
    ...
    return createCinematicSession({
        ...
        // The retrigger API is retired, but existing chat sessions retain this
        // bounded opaque cache so normal saves do not erase user data.
        ...(value.retriggerSuggestions ? { retriggerSuggestions: boundedMap(value.retriggerSuggestions, clone, 16) } : {}),
    });
}
...
export function createCinematicRuntime({
    ...
    function observe(input = {}) {
        return enqueue(async () => {
            ...
            if (!delta && typeof interpret === 'function') {
                try {
                    interpretation = await interpret({ ...input, priorStoryState: source.storyState });
                } catch (error) {
                    const failed = remember({
                        ...source,
                        queuedEvents: source.queuedEvents.filter((event) => event?.messageId !== input.messageId || event?.fingerprint !== fingerprint(input.message)),
                        lastResult: { status: 'failed', suggestion: null, nextTrigger: null },
                    });
                    if (activeChat(failed.chatId)) current = failed;
                    try { await persist(failed); } catch { /* failure state remains evictable in memory */ }
                    return { status: 'failed', error, suggestion: null };
                }
            }
            ...
            if (interpreted?.storyStateDelta) {
                ...
                const ambiguous = Boolean(interpreted.ambiguities?.length || interpreted.location?.status === 'ambiguous');
                delta = {
    ...
    return {
        load, getState, observe, adjust, dismiss, stage, approve,
