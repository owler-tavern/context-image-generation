...
function memoryDelta(before, after) {
    ...
    return {
        addedArtifactIds: sortedIds(newArtifacts).filter((id) => !oldArtifacts[id]),
        changedArtifactIds: sortedIds(newArtifacts).filter((id) => oldArtifacts[id] && !sameValue(oldArtifacts[id], newArtifacts[id])),
        removedArtifactIds: sortedIds(oldArtifacts).filter((id) => !newArtifacts[id]),
    };
}
...
export function createStoryMemoryController(dependencies = {}) {
    ...
    async function load({ chatId, gallery = [] } = {}) {
        ...
        try {
            ...
            const delta = memoryDelta(base, memory);
            const expected = { operation: 'hydrate-gallery', chatId: chatId ?? state.chatId ?? null, ...delta };
            const confirmed = await persistAndReadback(memory, { operation: 'hydrate-gallery', expected, delta });
            return update({ ...confirmed, status: 'ready', message: Object.keys(memory.artifacts).length ? 'Story memory loaded.' : 'No scene moments have been saved yet.', error: null, chatId: chatId ?? state.chatId ?? null, lastOperation: 'load' });
        } catch (error) {
    ...
    async function mutateArtifact(artifactId, operation, mutate) {
        ...
        try {
            ...
            return await persistAndReadback(next, { operation, artifactId: identity.artifactId, artifactVersion: identity.artifactVersion, expected });
    ...
    async function createCollection({ collectionId, kind = 'moment', label } = {}) {
        ...
        try {
            ...
            return await persistAndReadback(next, { operation: 'create-collection', collectionId: id, expected });
        }
    ...
    async function collectionMember(operation, collectionId, artifactId) {
        ...
        try {
            ...
            return await persistAndReadback(next, { operation, collectionId: id, artifactId: identity.artifactId, artifactVersion: identity.artifactVersion, expected });
        } catch (error) { return failure(error, operation); }
    }
    ...
    async function continueFromScene(artifactId, options = {}) {
        ...
        try {
            ...
            const receiptId = result.artifactId ?? result.sourceArtifactId;
            const receiptVersion = result.artifactVersion ?? result.sourceArtifactVersion;
            if (receiptId !== identity.artifactId) throw new TypeError('Continue planner receipt is not bound to the selected artifact.');
            if (receiptVersion !== identity.artifactVersion) throw new TypeError('Continue planner receipt has a different artifact version.');
            if (!result.authorityToken && result.requestId !== requestId) throw new TypeError('Continue planner receipt is missing the exact requestId or authority token.');
...
export function mountStoryMemorySurface(host, controller, options = {}) {
    ...
    const ownedStyle = options.installStyles === true ? installStoryMemoryStyles(options.documentLike || globalThis.document) : null;
    const click = (event) => {
    ...
    const submit = (event) => {
        ...
        if (!form) return;
        event.preventDefault?.();
        if (form.getAttribute('data-story-form') === 'filters') { filterDraft = { prompt: read('[name="prompt"]', filterDraft.prompt), characterId: read('[name="characterId"]', filterDraft.characterId), model: read('[name="model"]', filterDraft.model), from: read('[name="from"]', filterDraft.from), to: read('[name="to"]', filterDraft.to), favorite: read('[name="favorite"]', filterDraft.favorite) }; controller.setFilters(filterDraft, { replace: true }); }
    ...
    return { render, destroy() { unsubscribe(); host.removeEventListener('click', click); host.removeEventListener('input', input); host.removeEventListener('change', change); host.removeEventListener('submit', submit); if (ownedStyle) uninstallStoryMemoryStyles(options.documentLike || globalThis.document); } };
}
