...
export function createStoryMemoryController(dependencies = {}) {
    ...
    async function load({ chatId, gallery = [] } = {}) {
        ...
        state = { ...state, chatId: targetChatId, status: 'loading' };
        ...
        try {
            ...
            return update({ ...confirmed, status: 'ready', message: Object.keys(memory.artifacts).length ? 'Story memory loaded.' : 'No scene moments have been saved yet.', error: null, chatId: targetChatId, lastOperation: 'load' });
        } catch (error) {
...
export function mountStoryMemorySurface(host, controller, options = {}) {
    ...
    return { render, destroy() { unsubscribe(); host.removeEventListener('click', click); host.removeEventListener('input', input); host.removeEventListener('change', change); host.removeEventListener('submit', submit); if (ownedStyle) { const ownership = STYLE_OWNERS.get(styleDocument); if (ownership?.style === ownedStyle) ownership.owners = Math.max(0, ownership.owners - 1); if (!ownership || ownership.owners === 0) uninstallStoryMemoryStyles(styleDocument); } } };
}
