...
function normalizeWandPreferences(value) {
    if (!isRecord(value)) return null;
    // Preserve bounded legacy data without interpreting or defaulting retired controls.
    const { castOverrides: ignoredCast, stagedSuggestion: ignoredStage, ...stored } = value;
    const legacy = safeClone(stored) || {};
    ...
    return { ...legacy, ...(castOverrides.length ? { castOverrides } : {}), ...(staged?.suggestionId && staged.shot ? { stagedSuggestion: staged } : {}) };
}
...
export function migrateChatCanon(value) {
    ...
    // Outfit state is retired compatibility data. Preserve its exact persisted
    // value without traversing or interpreting it; this keeps the generic
    // unknown-field budget bounded while avoiding deletion of valid histories.
    if (Object.prototype.hasOwnProperty.call(source, 'outfitState')) state.outfitState = source.outfitState;
    let unknownBytes = 0;
    for (const [key, item] of Object.entries(source)) {
        if (key === 'schema' || key === 'bindings' || key === 'appearanceSources' || key === 'identityPins' || key === 'wandPreferences' || key === 'outfitState') continue;
