...
export function finalizeVisibleCanonPendingReplay({ currentCanon, replayedCanon, activePending } = {}) {
    ...
    const finalized = { ...candidate, visibleCanonPending: createVisibleCanonPendingState({ pending: activePending }).pending };
    // Visible-canon replay is unrelated to retired outfit data. The live canon
    // remains authoritative even when a historical replay candidate omitted it
    // or contains an older value.
    if (Object.prototype.hasOwnProperty.call(currentCanon || {}, 'outfitState')) finalized.outfitState = currentCanon.outfitState;
    return finalized;
}
