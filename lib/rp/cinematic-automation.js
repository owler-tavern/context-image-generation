...
export function evaluateCinematicAutomation({ session: inputSession, acceptedSceneDelta } = {}) {
    ...
    const registered = createCinematicSession({ ...session, eventRegistry, seenEventIds: Object.keys(eventRegistry) }); const pending = Object.values(registered.pendingSuggestions).find((suggestion) => events.some((event) => suggestion.eventIds?.includes(event.eventId)));
    if (!fresh.length || pending) return freeze({ status: 'duplicate-suppressed', session: registered, events, suggestion: pending || null, nextTrigger: nextTrigger(registered, unconsumedEvents(registered)) });
...
export function approveCinematicSuggestion(inputSession, suggestionValue, { estimatedCost } = {}) {
    ...
    const plan = { ...planFor(session, suggestion, event), estimatedCost: cost }; const pendingSuggestions = { ...session.pendingSuggestions }; delete pendingSuggestions[suggestion.suggestionId]; const reservation = { planId: plan.planId, suggestionId: suggestion.suggestionId, eventIds: suggestion.eventIds, reservedCost: cost, status: 'reserved' }; const consumedEventIds = [...new Set([...session.consumedEventIds, ...suggestion.eventIds])];
...
export function retriggerCinematicBeat({ session: inputSession, beatId, retriggerId = 'manual', reason = 'manual retrigger for missed or failed beat' } = {}) {
    const session = createCinematicSession(inputSession || {}); const beat = cleanText(beatId); const retry = cleanText(retriggerId); if (!beat || !retry) return { status: 'invalid-retrigger', session, suggestion: null }; const key = `${beat.length}:${beat}|${retry.length}:${retry}`; if (session.retriggerSuggestions[key]) return freeze({ status: 'suggested', session, suggestion: session.retriggerSuggestions[key] }); const event = { eventId: stableId('manual-event', { sessionId: session.sessionId, beatId: beat, retriggerId: retry }), beatId: beat, kind: 'emotional-beat', score: 3, change: { id: beat, label: beat, confidence: null }, whyFired: { reason: 'manual retrigger', text: `Manual retrigger requested: ${cleanText(reason) || 'missed or failed beat'}.`, evidence: [] } }; const suggestion = createSuggestion(session, event, [event], 'manual-retrigger', { beatId: beat, retriggerId: retry }); const nextSession = createCinematicSession({ ...session, retriggerSuggestions: { ...session.retriggerSuggestions, [key]: suggestion } }); return freeze({ status: 'suggested', session: nextSession, suggestion });
}
