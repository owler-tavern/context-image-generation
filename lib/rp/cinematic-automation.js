...
const CHANGE_SCORES = Object.freeze({ location: 3, 'emotional-beat': 3, cast: 2 });
const CHANGE_ORDER = Object.freeze(['location', 'cast', 'emotional-beat']);
...
function isAmbiguous(delta, seen = new Set()) {
    ...
    return Object.entries(delta).some(([key, value]) => key !== 'outfits' && isAmbiguous(value, seen));
}
...
function changedValue(delta, key) {
    ...
    if (key === 'location') {
        const locations = [updated, direct, added].filter((value) => isRecord(value) || typeof value === 'string').map((value) => sanitizeChange('location', typeof value === 'string' ? { from: null, to: value } : value)).filter(Boolean);
...
export function deriveCinematicEvents(value = {}) {
    ...
    const cast = changedValue(delta, 'cast'); if (cast && !cast.ambiguous) events.push(eventFor(revision, 'cast', cast, evidence));
...
export function approveCinematicSuggestion(inputSession, suggestionValue, { estimatedCost } = {}) {
    ...
    const countOnly = session.generationLimit !== null && session.costCeiling === null;
    const cost = validNonNegativeNumber(estimatedCost);
    if (!countOnly && cost === null) return freeze({ status: 'unknown-cost', session, plan: null });
    if (!countOnly && session.costCeiling === null) return freeze({ status: 'unknown-cost-ceiling', session, plan: null });
    if (session.generationLimit !== null && session.generationCount + session.reservedGenerationCount >= session.generationLimit) return freeze({ status: 'generation-limit', session, plan: null });
    if (!countOnly && session.costSpent + session.reservedCost + cost > session.costCeiling) return freeze({ status: 'cost-ceiling', session, plan: null });
    const reservedCost = countOnly ? 0 : cost;
    const plan = { ...planFor(session, suggestion, event), estimatedCost: countOnly ? null : cost }; const pendingSuggestions = { ...session.pendingSuggestions }; delete pendingSuggestions[suggestion.suggestionId]; const reservation = { planId: plan.planId, planDigest: stableJson(plan), suggestionId: suggestion.suggestionId, eventIds: suggestion.eventIds, reservedCost, status: 'reserved' }; const consumedEventIds = [...new Set([...session.consumedEventIds, ...suggestion.eventIds])];
    const nextSession = createCinematicSession({ ...session, pendingSuggestions, consumedEventIds, reservedGenerationCount: session.reservedGenerationCount + 1, reservedCost: session.reservedCost + reservedCost, approvedSuggestionIds: [...session.approvedSuggestionIds, suggestion.suggestionId], approvedSuggestionRecords: { ...session.approvedSuggestionRecords, [suggestion.suggestionId]: clone(suggestion) }, approvedPlans: { ...session.approvedPlans, [suggestion.suggestionId]: plan }, reservations: { ...session.reservations, [plan.planId]: reservation } }); return freeze({ status: 'approved', session: nextSession, plan: freeze(plan) });
}
...
export function settleCinematicPlan(inputSession, planValue, outcome, { actualCost } = {}) {
    ...
    if (normalizedOutcome === 'completed') { const countOnly = session.generationLimit !== null && session.costCeiling === null; const cost = countOnly ? 0 : validNonNegativeNumber(actualCost); if (cost === null) return freeze({ status: 'unknown-cost', session, plan }); const available = session.costCeiling === null ? null : session.costCeiling - session.costSpent - (session.reservedCost - reservation.reservedCost); if (!countOnly && available === null) return freeze({ status: 'unknown-cost-ceiling', session, plan }); if (!countOnly && cost > available) return freeze({ status: 'cost-ceiling', session, plan }); const released = releaseReservation(session, reservation); const reservations = { ...released.reservations }; delete reservations[plan.planId]; const settledPlans = { ...released.settledPlans, [plan.planId]: { outcome: 'completed', actualCost: countOnly ? null : cost } }; const nextSession = createCinematicSession({ ...released, reservations, settledPlans, generationCount: released.generationCount + 1, costSpent: released.costSpent + cost }); return freeze({ status: 'completed', session: nextSession, plan: session.approvedPlans[plan.suggestionId] || plan }); }
