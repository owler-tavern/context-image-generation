...
export async function persistOrphanCleanupRecovery({ library, url, saveLibrary, verifySaved, scheduleRetry, uuid = () => crypto.randomUUID() } = {}) {
    const candidate = clone(library || {});
    const operationId = `orphan-cleanup:${uuid()}`;
    candidate.operations = { ...(candidate.operations || {}), [operationId]: { status: 'orphan-cleanup', url } };
    candidate.revision = `appearance-orphan:${uuid()}`;
    let verification;
    try {
        await saveLibrary(candidate);
        verification = await verifySaved(candidate.revision);
    } catch (error) { verification = { status: 'indeterminate', error }; }
    if (verification?.status === 'confirmed') return { status: 'tracked', library: candidate };
    scheduleRetry?.({ operationId, library: candidate });
    return { status: 'recovery-pending', library: candidate, verification };
}
