...
import { finalizeGlobalLookDeletion } from './appearance-library.js';
...
export async function reconcileAppearanceOperations({ library, verifyRevision, saveLibrary, verifySaved, deleteFile = async () => {}, uuid = () => crypto.randomUUID() } = {}) {
    ...
    for (const [operationId, operation] of Object.entries(candidate.operations || {})) {
        ...
        } else if (operation?.status === 'deleting') {
            try {
                if (operation.url) await deleteFile(operation.url);
                const finalized = finalizeGlobalLookDeletion(candidate, operationId);
                Object.keys(candidate).forEach((key) => delete candidate[key]);
                Object.assign(candidate, finalized);
            } catch { /* tombstone remains unavailable and reload-retryable */ }
