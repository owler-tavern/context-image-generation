...
export function buildIdentityCatalogue({
    ...
    chatIdentities = [],
    currentChatId,
    // Omitted means legacy callers still receive the complete library. The
    // appearance-choice path passes an explicit array to enforce chat scope.
    allowedLibraryIdentityIds = null,
} = {}) {
    ...
    const hasExplicitAllowlist = Array.isArray(allowedLibraryIdentityIds);
    const allowed = new Set((hasExplicitAllowlist ? allowedLibraryIdentityIds : []).map((id) => String(id || '').trim()).filter(Boolean));
    ...
    for (const identity of libraryIdentities) {
        ...
        const isCurrentChatNpc = identity?.kind === 'npc' && currentChatId !== undefined && String(identity.chatId || '') === String(currentChatId || '');
        if (hasExplicitAllowlist && !allowed.has(id) && !isCurrentChatNpc) continue;
        add(identityRecord(identity, identity.id));
    }
