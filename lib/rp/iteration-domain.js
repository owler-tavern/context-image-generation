...
function hasVerifiedResolvedRoute(route) {
    return isRecord(route)
        && (route.status === 'verified' || route.verified === true)
        && route.resolved === true
        && (route.capabilities?.imageGeneration === true || route.capabilityTripwire === true);
}

function hasCapabilityTripwire(value) {
    return value === true || value?.status === 'verified' && (value.imageGeneration === true || value.capabilities?.imageGeneration === true);
}
...
function normalizeConsent(value, outputCount) {
    ...
    const acknowledgedUnavailable = consent.costUnavailableAcknowledged === true || consent.costUnavailable === true;
...
export function planIterationAction({
    ...
    const routeReady = hasVerifiedResolvedRoute(resolvedRoute) || (
        isRecord(resolvedRoute)
        && (resolvedRoute.status === 'verified' || resolvedRoute.verified === true)
        && resolvedRoute.resolved === true
        && hasCapabilityTripwire(capabilityTripwire)
    );
    const status = isDraft ? 'draft-review' : isCanonUpdate ? 'canon-ready' : consentMatches ? 'ready' : 'consent-required';
    const dispatch = {
        allowed: status === 'ready' && routeReady,
        network: false,
        reason: isDraft ? 'manual-confirmation-required' : isCanonUpdate ? 'canon-update-only' : !routeReady ? 'verified-route-and-capability-required' : status === 'ready' ? null : 'explicit-paid-consent-required',
    };
