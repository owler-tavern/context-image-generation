...
const SAFE_RECIPE_REFERENCE_KEYS = new Set(['id', 'role', 'identityId', 'assetId', 'label']);
const SAFE_RECIPE_ROUTE_KEYS = new Set(['providerId', 'modelId', 'transportId', 'connectionId', 'endpointClass', 'revision']);
const SAFE_RECIPE_MODEL_KEYS = new Set(['providerId', 'modelId', 'transportId', 'connectionId', 'endpointClass']);
const SAFE_RECIPE_OPTION_KEYS = new Set(['aspectRatio', 'imageSize', 'thinkingLevel', 'useGoogleSearch', 'seed', 'quality', 'style', 'size', 'width', 'height']);
...
function pickKeys(value, keys) {
    ...
    return Object.fromEntries([...keys].filter((key) => value[key] !== undefined).map((key) => [key, value[key]]).filter(([, item]) => typeof item !== 'string' || !/^data:|https?:\/\//iu.test(item)).map(([key, item]) => [key, typeof item === 'string' ? bounded(item, 160) : typeof item === 'number' || typeof item === 'boolean' ? item : undefined]).filter(([, item]) => item !== undefined));
}

/**
 * Returns the bounded, replay-authoritative metadata safe to persist on chat
 * media. Prompt/context/provider messages, assets, URLs, and credentials are
 * deliberately omitted. The full recipe remains transient for the current
 * dispatch only; a reloaded artifact must be re-created from a live source.
 */
export function sanitizeIterationArtifactForStorage(value = {}) {
    const artifact = isRecord(value) ? value : {};
    const recipe = isRecord(artifact.recipe) ? artifact.recipe : artifact;
    const sourcePassage = isRecord(recipe.sourcePassage) ? recipe.sourcePassage : {};
    const references = normalizeReferences(recipe.references)
        .map((reference) => pickKeys(reference, SAFE_RECIPE_REFERENCE_KEYS))
        .filter((reference) => reference.id || reference.identityId || reference.assetId);
    const safeOptions = pickKeys(recipe.options, SAFE_RECIPE_OPTION_KEYS);
    const referenceReceipt = (Array.isArray(artifact.referenceReceipt?.used) || Array.isArray(artifact.referenceReceipt?.omitted))
        ? compactReferenceReceipt(artifact.referenceReceipt)
        : null;
    if (isRecord(recipe.options?.composition)) safeOptions.composition = Object.fromEntries(Object.entries(recipe.options.composition).slice(0, 8).map(([key, item]) => [bounded(key, 40), bounded(item, 80)]));
    const authority = isRecord(artifact.generationPlan) ? artifact.generationPlan : {};
    const safeGenerationPlan = {
        planId: bounded(authority.planId, 180),
        revision: bounded(authority.revision || authority.routeRevision, 180),
        routeConfirmationAccepted: authority.routeConfirmationAccepted === true,
        capabilities: isRecord(authority.capabilities) ? { imageGeneration: authority.capabilities.imageGeneration === true } : {},
    };
    const sourceText = sourcePassage.userVisible === true && !/\bdata:/iu.test(String(sourcePassage.text || ''))
        ? bounded(sourcePassage.text, 4000)
        : '';
    return {
        schema: 1,
        artifactId: bounded(artifact.artifactId, 180),
        parentArtifactId: artifact.parentArtifactId ? bounded(artifact.parentArtifactId, 180) : null,
        action: bounded(artifact.action, 80),
        createdAt: Number.isFinite(artifact.createdAt) ? artifact.createdAt : null,
        recipeAvailable: Boolean(sourceText && bounded(recipe.effectivePrompt, 4000) && (recipe.route?.providerId || recipe.model?.providerId) && (recipe.route?.modelId || recipe.model?.modelId)),
        sourcePassage: {
            messageId: bounded(sourcePassage.messageId, 120) || null,
            chatId: bounded(sourcePassage.chatId, 120) || null,
            digest: fnv1a64(bounded(sourcePassage.text, 12000)),
            ...(sourceText ? { text: sourceText, userVisible: true } : {}),
        },
        effectivePromptDigest: fnv1a64(bounded(recipe.effectivePrompt, 16000)),
        effectivePrompt: bounded(recipe.effectivePrompt, 4000),
        references,
        model: pickKeys(recipe.model, SAFE_RECIPE_MODEL_KEYS),
        route: pickKeys(recipe.route, SAFE_RECIPE_ROUTE_KEYS),
        options: safeOptions,
        generationPlan: safeGenerationPlan.planId && safeGenerationPlan.revision ? safeGenerationPlan : null,
        canonSnapshot: {
            continuityRevision: bounded(recipe.canonSnapshot?.continuityRevision, 120) || null,
        },
...
function artifactInput(value = {}) {
    ...
    const sourcePassage = clone(recipe.sourcePassage || {});
    return {
        recipeAvailable: value.recipeAvailable !== false && recipe.recipeAvailable !== false && Boolean(cleanPrompt(sourcePassage.text)),
        sourcePassage,
...
function canonicalRoles(changes) {
    ...
    for (const [key, value] of Object.entries(sourceRoles)) {
        ...
        if (normalizedKey === 'activeLook' && (!isRecord(value) || !cleanPrompt(value.identityId))) throw new TypeError('Canonical activeLook requires identityId.');
...
function deriveRecipe(action, base, changes) {
    ...
    switch (action) {
        ...
        case ACTIONS.REUSE_RECIPE:
            if (base.recipeAvailable === false || !recipe.effectivePrompt) throw new TypeError('Saved recipe is unavailable; open the original image to capture a fresh recipe.');
...
export function planIterationAction({
    ...
    if (typeof releaseInvocation !== 'function') throw new TypeError('releaseInvocation is required to release invocation ownership.');
    ...
    const consentMatches = (allowUnquotedSingle && outputCount === 1) || (consent.approved
