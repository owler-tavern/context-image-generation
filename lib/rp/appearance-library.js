...
function normalizeIdentityLooks(identity) {
    ...
    return {
        ...clone(identity),
        looks: looks.map((look) => normalizeLook(look, look?.id || `look:${identity.id}`, look?.assetId))
            .filter((look) => {
...
export function listAppearanceIdentityChoices({ activeCharacter, persona, groupMembers = [], chatIdentities = [] } = {}) {
    return buildIdentityCatalogue({ activeCharacter, persona, groupMembers, chatIdentities });
}
...
export function buildAppearanceReferenceCandidates(libraryValue, gallery = []) {
    const library = migrateAppearanceLibrary(libraryValue);
