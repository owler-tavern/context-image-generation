...
export function stageLegacyArtifactMigration(libraryValue, { operationId, galleryArtifactId, targetAssetId, targetUrl } = {}) {
    const library = migrateAppearanceLibrary(libraryValue);
    const legacyAsset = Object.values(library.assets).find((asset) => asset?.kind !== 'appearance' && String(asset?.source?.galleryId || '') === String(galleryArtifactId || ''));
    if (!legacyAsset || !operationId || !targetAssetId || !targetUrl) throw new TypeError('A legacy Gallery artifact and deterministic migration target are required.');
    const mappings = [];
    for (const [identityId, identity] of Object.entries(library.identities)) {
        for (const look of identity.looks || []) if (look.assetId === legacyAsset.id) mappings.push({ identityId, lookId: look.id });
    }
    ...
    library.operations = { ...(library.operations || {}), [operationId]: {
        ...
        sourceAssetId: legacyAsset.id, targetAssetId: String(targetAssetId), targetUrl: String(targetUrl), mappings,
    } };
...
export function finalizeLegacyArtifactMigration(libraryValue, operationId, { mimeType, byteCount, createdAt = Date.now() } = {}) {
    ...
    for (const mapping of operation.mappings || []) {
        ...
        if (look && look.assetId === operation.sourceAssetId) look.assetId = operation.targetAssetId;
    }
    const stillUsed = Object.values(library.identities).some((identity) => (identity.looks || []).some((look) => look.assetId === operation.sourceAssetId));
    if (!stillUsed) delete library.assets[operation.sourceAssetId];
    delete library.operations[operationId];
    library.schema = 2;
    return library;
}
