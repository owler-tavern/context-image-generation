...
export function applyGalleryClear({ gallery = [], library: libraryValue = {}, confirmed } = {}) {
    ...
    for (const identity of Object.values(library.identities)) {
        identity.looks = [];
        identity.activeLookId = null;
    }
    library.assets = {};
    return { decision: 'cleared', gallery: [], library };
}
...
export function materializeAppearanceAssets(libraryValue, gallery = []) {
    const library = migrateAppearanceLibrary(libraryValue);
    const suppressedAssetIds = new Set(Object.values(library.operations || {}).filter((operation) => operation?.status === 'pending' || operation?.status === 'deleting').map((operation) => operation.assetId));
