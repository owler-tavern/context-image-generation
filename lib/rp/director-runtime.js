...
function castCandidates(value) {
    return (Array.isArray(value) ? value : []).slice(0, 12).map((candidate) => ({
        ...
        action: ['auto', 'include', 'focus', 'exclude'].includes(candidate?.action) ? candidate.action : 'include',
        inferredAction: ['auto', 'include', 'focus', 'exclude'].includes(candidate?.inferredAction) ? candidate.inferredAction : 'include',
...
export function createDirectorRuntime({
    ...
    const chatStates = createBoundedRecencyMap({
        maxEntries: 32,
        isProtected: (chatId) => active(chatId) || String(chatId) === inFlightChatId,
    });
    ...
    async function generate() {
        ...
        if (inFlight) return { status: 'busy' };
        inFlight = true;
        inFlightChatId = String(state.chatId);
        const route = await validateRoute({ target: clone(target), panel: clone(panel), revision: state.revision });
        if (route?.allowed !== true) { inFlight = false; inFlightChatId = null; return { status: route?.status || 'route-invalid', reason: route?.reason }; }
        const revision = state.revision;
        const chatKey = String(state.chatId);
        if (chatStates.get(chatKey)?.revision !== revision || !currentEnough(target)) { inFlight = false; inFlightChatId = null; return { status: 'stale', reason: 'draft-changed' }; }
        try {
        ...
        } finally { inFlight = false; inFlightChatId = null; }
    }
    return { load, getState, open, update, close, generate, destroy() { current = null; chatStates.clear(); inFlight = false; inFlightChatId = null; } };
}
