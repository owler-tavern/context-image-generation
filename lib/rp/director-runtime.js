...
export function createDirectorRuntime({
    ...
    async function generate() {
        ...
        try {
            const route = await validateRoute({ target: clone(target), panel: clone(panel), revision });
            if (route?.allowed !== true) return { status: route?.status || 'route-invalid', reason: route?.reason };
            if (chatStates.get(chatKey)?.revision !== revision || !currentEnough(target)) return { status: 'stale', reason: 'draft-changed' };
