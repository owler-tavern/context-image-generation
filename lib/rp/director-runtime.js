...
export function createDirectorRuntime({
    ...
    const persist = async (state = current) => {
        if (!state) return;
        remember(state);
        writeState({ [DIRECTOR_STATE_KEY]: safePersistState(state) }, { chatId: state.chatId });
        if (active(state.chatId)) await saveChat({ chatId: state.chatId });
    };
    function load({ chatId = getChatId(), epoch = getEpoch() } = {}) {
        const key = String(chatId ?? '');
        current = chatStates.get(key) || initialState(key, readState({ chatId: key }));
        remember(current);
    ...
    async function open(input = {}) {
        ...
        const nextOptions = options({ ...state.options, ...(input.options || {}) });
        let preview;
        try {
            preview = await buildPreview({ sourceMessage, focusText, target: clone(target), options: nextOptions });
        } catch (error) {
        ...
        current = remember({ ...state, options: nextOptions, revision: state.revision + 1, lastStatus: 'open', panel: panelFor({
            messageId: target.messageId, target, sourceMessage, focusText, ...nextOptions,
            ...clone(preview), budgetSummary: preview?.budgetSummary || getBudgetSummary({ options: nextOptions }),
        }) });
    ...
    async function update(values = {}) {
        ...
        const nextOptions = options(input);
        let preview = {};
        try {
        ...
        current = remember({ ...state, options: nextOptions, revision: state.revision + 1, lastStatus: 'edited', panel: panelFor({ ...state.panel, ...nextOptions, ...clone(preview), budgetSummary: preview?.budgetSummary || getBudgetSummary({ options: nextOptions }) }) });
        await persist(current);
    ...
    async function generate() {
        ...
        try {
            const result = await dispatch({
                ...
                visualDirection: panel.visualDirection, revision, panel: clone(panel),
            });
