...
export function renderDirectorPanel(panel = {}) {
    ...
    const status = panel.status === 'failed' ? `<p role="alert">${escapeHtml(panel.previewError || 'Generation did not complete. Your draft is still here.')}</p>` : '';
...
<p><strong>Target:</strong> message ${messageId || 'this message'}${panel.focusText ? ' (selected passage)' : ''}</p>
...
${status}<div class="cig_director_actions"><button type="button" style="min-height:44px" data-director-action="generate"${panel.status === 'generating' ? ' disabled' : ''}>Generate directed image</button><button type="button" style="min-height:44px" data-director-action="close">Close Director</button></div>
</section>`;
}
