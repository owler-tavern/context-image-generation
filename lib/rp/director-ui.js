...
.cig_director_cast { margin:.75rem 0; padding:.625rem; border:1px solid var(--SmartThemeBorderColor,#777); border-radius:.4rem; }
.cig_director_cast_row { display:grid; grid-template-columns:minmax(7rem,1fr) repeat(3,minmax(5rem,auto)); gap:.375rem; align-items:center; margin-top:.5rem; }
.cig_director_cast_row button { width:auto; min-height:44px; }
@media (max-width:480px) { .cig_director_panel { padding:.625rem; } .cig_director_actions { display:grid; grid-template-columns:1fr; } .cig_director_cast_row { grid-template-columns:1fr 1fr; } .cig_director_cast_row strong, .cig_director_cast_row span { grid-column:1 / -1; } }
`;
...
/**
 * Move focus into the newly opened inline disclosure without imposing a
 * modal focus trap. The first meaningful control is framing, followed by the
 * normal DOM order for continuity, direction, generate, and close.
 */
export function focusDirectorPanel({ documentLike = globalThis.document, messageId } = {}) {
    const normalizedMessageId = Number(messageId);
    if (!documentLike || !Number.isFinite(normalizedMessageId)) return { status: 'unavailable' };
    const selector = `.cig_director_panel[data-message-id="${normalizedMessageId}"]`;
    const panel = documentLike.querySelector?.(selector);
    if (!panel) return { status: 'missing' };
    const target = panel.querySelector?.('[data-director-field="framing"]')
        || panel.querySelector?.('h3')
        || panel;
    if (target === panel && typeof panel.setAttribute === 'function') panel.setAttribute('tabindex', '-1');
    return focusWithoutScroll(target) ? { status: 'focused', target } : { status: 'unavailable' };
}

/** Restore focus only to the exact current opener captured for this panel. */
export function restoreDirectorTriggerFocus({ trigger, isCurrent = () => true } = {}) {
    if (!trigger || !isCurrent() || trigger.isConnected === false || trigger.disabled === true) return { status: 'not-restored' };
    return focusWithoutScroll(trigger) ? { status: 'restored', target: trigger } : { status: 'not-restored' };
}
...
export function renderDirectorPanel(panel = {}) {
    ...
    const candidates = Array.isArray(panel.castCandidates) ? panel.castCandidates.slice(0, 12) : [];
    const overrides = new Map((Array.isArray(panel.castOverrides) ? panel.castOverrides : []).map((entry) => [String(entry.identityId), entry.action]));
    const cast = candidates.length
        ? candidates.map((candidate) => {
            const id = escapeHtml(candidate.identityId);
            const label = escapeHtml(candidate.label || 'Character');
            const action = overrides.get(String(candidate.identityId)) || candidate.action || 'include';
            const stateLabel = action === 'focus' ? 'focus priority' : action === 'auto' ? 'auto (inferred)' : `${action}d`;
            return `<div class="cig_director_cast_row" data-director-cast-row data-director-cast-id="${id}" data-current-action="${escapeHtml(action)}"><strong>${label}</strong><span>Current: ${escapeHtml(stateLabel)}</span><button type="button" style="min-height:44px" data-director-cast-action="auto" data-director-cast-id="${id}" aria-pressed="${action === 'auto'}">Auto ${label}</button><button type="button" style="min-height:44px" data-director-cast-action="include" data-director-cast-id="${id}" aria-pressed="${action === 'include'}">Include ${label}</button><button type="button" style="min-height:44px" data-director-cast-action="focus" data-director-cast-id="${id}" aria-pressed="${action === 'focus'}">Focus ${label}</button><button type="button" style="min-height:44px" data-director-cast-action="exclude" data-director-cast-id="${id}" aria-pressed="${action === 'exclude'}">Exclude ${label}</button></div>`;
        }).join('')
        : '<p>No confident cast candidates were found. Existing story cast behavior will be preserved.</p>';
...
<section class="cig_director_cast" aria-labelledby="cig_director_cast_heading_${messageId}"><h4 id="cig_director_cast_heading_${messageId}">Cast correction</h4><p>Choose who should appear in this exact scene. Known identities default to Auto and stay unchanged until you choose Include, Focus, or Exclude.</p>${cast}</section>
