...
export function renderDirectorPanel(panel = {}) {
    ...
    const cast = candidates.length
        ? candidates.map((candidate) => {
            ...
            return `<div class="cig_director_cast_row" data-director-cast-row data-director-cast-id="${id}" data-current-action="${escapeHtml(action)}"><strong>${label}</strong><span>Current: ${escapeHtml(action === 'focus' ? 'focus priority' : `${action}d`)}</span><button type="button" style="min-height:44px" data-director-cast-action="include" data-director-cast-id="${id}" aria-pressed="${action === 'include'}">Include ${label}</button><button type="button" style="min-height:44px" data-director-cast-action="focus" data-director-cast-id="${id}" aria-pressed="${action === 'focus'}">Focus ${label}</button><button type="button" style="min-height:44px" data-director-cast-action="exclude" data-director-cast-id="${id}" aria-pressed="${action === 'exclude'}">Exclude ${label}</button></div>`;
        }).join('')
