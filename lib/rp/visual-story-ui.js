...
function scheduleFocusRetry(documentLike, attempts = 8) {
    const retry = (remaining) => {
        focusVisualStorySurface({ documentLike });
        if (remaining <= 0) return;
        setTimeout(() => retry(remaining - 1), 25);
    };
    if (typeof queueMicrotask === 'function') queueMicrotask(() => retry(attempts));
    else setTimeout(() => retry(attempts), 0);
}
...
export function revealVisualStorySurface({ documentLike = globalThis.document, activateTab, tab = 'images-cast' } = {}) {
    ...
    scheduleFocusRetry(documentLike);
