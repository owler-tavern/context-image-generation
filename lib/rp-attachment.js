import { sanitizeIterationArtifactForStorage } from './rp/iteration-domain.js';
...
export async function attachGeneratedImageSafely({
    ...
    const galleryMetadata = (result) => result?.__cigIterationArtifact
        ? { iterationArtifact: sanitizeIterationArtifactForStorage(result.__cigIterationArtifact) }
        : undefined;
    const persist = async (result, signal) => {
        ...
        if (!validation?.safe) {
            await addToGallery(result.imageData, prompt, target.messageId, filePath, {
                ...
                ...(galleryMetadata(result) ? { iterationArtifact: galleryMetadata(result).iterationArtifact } : {}),
            });
        ...
        const storedIterationArtifact = result?.__cigIterationArtifact
            ? sanitizeIterationArtifactForStorage(result.__cigIterationArtifact)
            : null;
        ...
        const saveResult = await saveChat(result);
        ...
        await addToGallery(result.imageData, prompt, target.messageId, filePath, galleryMetadata(result) ? { iterationArtifact: galleryMetadata(result).iterationArtifact } : undefined);
    ...
    return (await persist(generated)).attached;
}
