...
function requireRouteEvidence(plan, connection) {
    ...
    if (customValidation?.valid) {
        const revision = connectionRevision(customValidation.connection);
        ...
        if (evidence?.state === 'configured') {
            const manualInvocation = ['settings', 'wand', 'slash'].includes(plan?.invocation);
...
export async function dispatchProviderRoute({ plan, connection, signal, transportContext = {} } = {}) {
    ...
    if (isCustomConnection) {
        const projection = safeConnectionProjection(customValidation.connection);
        if (!customValidation.connection.enabled) throw new Error('Custom connection is disabled.');
        const gemini = customValidation.connection.protocol === 'gemini-compatible';
        const expectedTransport = gemini ? 'sillytavern-gemini-proxy' : 'openai-images';
        const expectedEndpointClass = gemini ? 'custom-gemini-proxy' : 'custom-openai-images';
        if (plan.resolved?.transportId !== expectedTransport || plan.resolved?.endpointClass !== expectedEndpointClass) throw new Error('Custom connection route does not match its configured protocol.');
        const expectedEndpoint = gemini ? projection.routePreview.generation.upstreamProxyRoot : projection.routePreview.generation.url;
        if (plan.resolved?.endpoint !== expectedEndpoint) throw new Error('Custom generation endpoint does not match the saved connection revision.');
    }
    ...
    const effectiveTransportContext = isCustomConnection
        ...
        : transportContext;
    const result = await adapter.generate({ plan, connection, signal, transportContext: effectiveTransportContext });
    return decodeProviderArtifact(result, { signal, fetchImpl: effectiveTransportContext.fetchImpl, downloadLimits: effectiveTransportContext.downloadLimits });
}

export function promoteCustomConnectionEvidence({ connection, evidence, decodedResult, now = () => new Date().toISOString() } = {}) {
...
export function promoteCustomModelEvidence({ connection, modelId, decodedResult, now = () => new Date().toISOString() } = {}) {
    const validation = validateCustomConnection(connection);
    ...
    return {
        state: 'supported',
        revision: connectionRevision(validation.connection),
        observedAt: now(),
    };
}
