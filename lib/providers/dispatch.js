...
async function requestSillyTavern(plan, connection, signal, context, requestBody) {
    ...
    // SillyTavern can report an upstream failure in an HTTP-200 JSON envelope.
    // Check it before interpreting the response as an image result.
    if (json?.error) {
        const message = typeof json.error?.message === 'string' ? json.error.message
            : typeof json.error === 'string' ? json.error : 'The host reported a generation error without details.';
        throw Object.assign(new Error(message), { code: 'HOST_GENERATION_ERROR' });
    }
...
export async function dispatchProviderRoute({ plan, connection, signal, transportContext = {} } = {}) {
    ...
    try {
        ...
        return await decodeWithTelemetry(rawArtifact, { signal, fetchImpl: effectiveTransportContext.fetchImpl, downloadLimits: effectiveTransportContext.downloadLimits, telemetry: effectiveTransportContext.telemetry });
    } catch (error) {
        if (error?.name === 'AbortError') throw error;
        throw attachNormalizedProviderError(error, {
            providerId: plan.resolved.providerId,
            modelId: plan.resolved.modelId,
            providerLabel: declaredProvider?.label || connection?.label || undefined,
        });
    }
}
