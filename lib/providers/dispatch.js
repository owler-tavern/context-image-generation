...
const openAiImages = {
    ...
    async generate({ plan, connection, signal, transportContext = {} }) {
        ...
        const size = sizesSupported && typeof transportContext.mapAspectRatioToSize === 'function' && plan.options?.aspectRatio
            ? transportContext.mapAspectRatioToSize(plan.options.aspectRatio)
            : sizesSupported ? plan.options?.imageSize : undefined;
...
export async function dispatchProviderRoute({ plan, connection, signal, transportContext = {} } = {}) {
    ...
    if (!plan) {
        ...
        if (transportId === 'openai-images' && typeof legacy.requestOpenAiImages === 'function') {
            const legacyPlan = {
                ...
                options: { aspectRatio: route.model?.supportsSize ? legacy.aspectRatio : '' },
            };
    ...
    if (declaredProvider?.available === false || declaredProvider?.posture === 'future-server') validateProviderRoute({
        providerId: plan.resolved?.providerId,
        modelId: plan.resolved?.modelId,
        transportId: plan.resolved?.transportId,
    });
    if (!transportContext.transports) validateProviderRoute({
        providerId: plan.resolved?.providerId,
        modelId: plan.resolved?.modelId,
        transportId: plan.resolved?.transportId,
    });
