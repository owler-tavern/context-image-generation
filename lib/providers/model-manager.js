...
import { connectionRevision, customModelRouteRevision, resolveCustomModelRoute, validateCustomConnection } from './custom-connections.js';
...
export function mergeCustomDiscoveryModelRecords(entries, result, inputConnection) {
    ...
    const provider = { id: connection.id, transportIds: ['openai-images', 'sillytavern-gemini-proxy'] };
    ...
    for (const candidate of result.models) {
        ...
        const route = resolveCustomModelRoute(connection, candidate);
        if (candidate?.connectionId !== connection.id
            || candidate?.providerId !== connection.id
            || !route
            || candidate?.transportId !== route.transportId
            || !['configured', 'verified'].includes(routeEvidence?.state)
            || routeEvidence?.revision !== (connection.generationMethods ? customModelRouteRevision(connection, candidate) : revision)) continue;
        routeSafeCount += 1;
...
export function getDiscoveryRefreshMessage(result = {}) {
    ...
    const unresolvedCount = Number.isInteger(evidence.unresolvedCount) && evidence.unresolvedCount > 0 ? evidence.unresolvedCount : 0;
    if (unresolvedCount > 0) return { level: acceptedCount > 0 ? 'success' : 'warning', message: `Catalog refreshed in Setup → Model: ${acceptedCount} with known image routes; ${unresolvedCount} marked Unverified. Unverified entries need a documented protocol in a custom connection before generation.` };
