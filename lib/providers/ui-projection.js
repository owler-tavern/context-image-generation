...
function projectCredentialUi(provider) {
    ...
    const requiresApiKey = ui.requiresApiKey === true;
    ...
    return {
        mode: requiresApiKey ? 'extension-key' : 'sillytavern',
        label,
        placeholder: requiresApiKey && typeof configured.placeholder === 'string' && configured.placeholder.trim()
            ? configured.placeholder.trim()
            : requiresApiKey ? 'Enter API key' : '',
        setupHelp,
