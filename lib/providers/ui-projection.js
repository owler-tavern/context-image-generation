...
import { capabilityIsSupported, normalizeCapabilityEvidence } from './contracts.js';
...
export function projectProviderUi(providerId, modelId, { localEntries = [] } = {}) {
    ...
    const referenceImageMaxCount = model?.referenceImages?.maxCount;
    return {
        id: provider.id,
        label: provider.label || provider.id,
        status: provider.status,
        ...
        supportsReferenceImages: model?.supportsReferenceImages !== false && Number.isInteger(referenceImageMaxCount) && referenceImageMaxCount > 0,
        ...(Number.isInteger(referenceImageMaxCount) && referenceImageMaxCount > 0 ? { referenceImageMaxCount } : {}),
