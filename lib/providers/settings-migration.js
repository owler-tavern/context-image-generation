...
export function migrateProviderSettings(input = {}) {
    ...
    for (const [connectionId, entry] of Object.entries(existingConnections)) {
        ...
        if (!provider || !entry || typeof entry !== 'object' || !String(connectionId).startsWith(`${providerId}:`) || (entry.kind && !['browser-api-key', 'sillytavern-proxy', 'server-adapter'].includes(entry.kind))) {
