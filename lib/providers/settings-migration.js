...
function safeDiscoveryEvidence(value) {
    ...
    for (const key of ['returnedCount', 'acceptedCount', 'unresolvedCount', 'rejectedCount']) {
        if (Number.isInteger(value[key]) && value[key] >= 0 && value[key] <= 100000) evidence[key] = value[key];
    }
...
function safeExperimentalModelPreflight(value) {
    const input = validObject(value);
    const result = {};
    for (const [key, accepted] of Object.entries(input)) {
        if (typeof accepted !== 'boolean') continue;
        let parsed;
        try { parsed = JSON.parse(key); } catch { continue; }
        if (!Array.isArray(parsed) || parsed.length !== 3 || parsed.some((part) => typeof part !== 'string' || !part.trim()) || JSON.stringify(parsed) !== key) continue;
        result[key] = accepted;
    }
    return result;
}
...
export function migrateProviderSettings(input = {}) {
    ...
    delete settings.linkapi_use_legacy_routing;
    ...
    settings.experimental_model_preflight = safeExperimentalModelPreflight(settings.experimental_model_preflight);
    settings.custom_connections = migrateCustomConnections(settings.custom_connections);
    const customKeys = validObject(settings.custom_connection_keys);
    const referencedCustomCredentials = new Set(Object.values(settings.custom_connections.connections)
        .map((connection) => connection.credentialRef)
        .filter(Boolean));
    settings.custom_connection_keys = Object.fromEntries(Object.entries(customKeys)
        .filter(([credentialRef, value]) => referencedCustomCredentials.has(credentialRef) && typeof value === 'string' && value.length <= 4096));
