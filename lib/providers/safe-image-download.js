...
async function readResponseBytes(response, maxBytes, signal) {
    // A one-shot arrayBuffer() cannot be bounded while it is being allocated.
    // Require a reader so the cap is enforced before accepting each chunk.
    if (!response.body?.getReader) throw new TypeError('Bounded image response stream is required.');
    ...
    try {
        while (true) {
            ...
            if (total > maxBytes) {
                await reader.cancel?.();
                throw new RangeError('Image exceeds the size limit.');
            }
            chunks.push(chunk);
        }
    } finally {
...
export async function downloadImageData(rawUrl, {
    ...
    if (rawUrl.startsWith('data:')) throw new TypeError('Data URLs must be decoded by the artifact decoder.');
    let currentUrl;
    ...
    try {
        for (let redirectCount = 0; ; redirectCount += 1) {
            ...
            const buffer = await readResponseBytes(response, maxBytes, request.signal);
            throwIfAborted(request.signal);
