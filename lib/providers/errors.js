...
function classify(message, status, error) {
    ...
    if (error?.code === 'HOST_GENERATION_ERROR') return 'provider_response';
...
export function normalizeProviderError(error, {
    ...
    if (error?.code === 'AVATAR_REFERENCES_UNAVAILABLE') {
        return {
            category: 'reference_unavailable',
            userMessage: 'The selected avatar references could not be loaded. Check the character or persona avatar and try again.',
            technicalMessage: redact(error.message),
            providerId,
            modelId,
            code: error.code,
        };
    ...
    return {
        category,
        userMessage: category === 'provider_response' ? `${providerLabel} returned an error: ${redact(rawMessage)}` : userMessage(category, providerLabel),
