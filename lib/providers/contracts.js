...
const ROUTE_EVIDENCE_STATES = new Set(['verified', 'configured', 'unverified']);
const ROUTE_EVIDENCE_SOURCES = new Set(['built-in', 'official-docs', 'sanitized-probe', 'user-configured-protocol']);
const ROUTE_PROTOCOLS = new Set(['openai-images', 'gemini-compatible', 'zai-native', 'arliai-native']);
const ROUTE_REQUEST_SHAPES = new Map([
    ['openai-images', 'openai-images-v1'],
    ['gemini-compatible', 'st-gemini-proxy-v1'],
    ['zai-native', 'zai-native-v1'],
    ['arliai-native', 'arliai-native-v1'],
]);
const ROUTE_EVIDENCE_MAX_AGE_MS = 90 * 24 * 60 * 60 * 1000;
...
export function normalizeRouteEvidence(value = {}) {
    const state = ROUTE_EVIDENCE_STATES.has(value?.state) ? value.state : 'unverified';
    if (state === 'unverified') return { state: 'unverified' };
    const source = ROUTE_EVIDENCE_SOURCES.has(value?.source) ? value.source : '';
    const protocol = ROUTE_PROTOCOLS.has(value?.protocol) ? value.protocol : '';
    const observedAt = typeof value?.observedAt === 'string' && !Number.isNaN(Date.parse(value.observedAt))
        ? value.observedAt
        : '';
    const requestShapeRevision = typeof value?.requestShapeRevision === 'string' ? value.requestShapeRevision : '';
    const isFresh = observedAt && Date.now() - Date.parse(observedAt) <= ROUTE_EVIDENCE_MAX_AGE_MS;
    if (!source || !protocol || !observedAt || !isFresh || ROUTE_REQUEST_SHAPES.get(protocol) !== requestShapeRevision) {
        return { state: 'unverified' };
    }
    const revision = typeof value?.revision === 'string' && /^connection-revision:[a-f0-9]{16}$/u.test(value.revision) ? value.revision : undefined;
    return { state: source === 'user-configured-protocol' ? 'configured' : state, source, observedAt, protocol, requestShapeRevision, ...(revision ? { revision } : {}) };
}
...
export function unknownCapabilities() {
    return {
        ...
        responseFormat: normalizeCapabilityEvidence(),
...
export function normalizeModelCapabilities(value = {}, legacy = {}, { allowLegacyCurated = false } = {}) {
    ...
    const result = {
        ...
        responseFormat: legacyCapability(source.responseFormat ?? legacy.responseFormat ?? legacy.supportsResponseFormat, { allowLegacyCurated }),
    ...
    if (capabilityIsSupported(result.sizes) && Array.isArray(allowedSizes)) result.allowedSizes = allowedSizes.filter((item) => typeof item === 'string' && item);
...
export function normalizeModelDefinition(input = {}, provider = {}) {
    ...
    const explicitTransport = input.transportId || input.transport || null;
    const transportId = explicitTransport || (source.kind === 'built-in' ? availableTransports.at(0) || null : null);
    ...
    const model = {
        ...
        transportId,
        routeEvidence: (() => {
            const evidence = normalizeRouteEvidence(input.routeEvidence);
            return source.kind !== 'built-in' && evidence.source === 'built-in' ? { state: 'unverified' } : evidence;
        })(),
    ...
    if (typeof input.connectionId === 'string' && input.connectionId.trim()) model.connectionId = input.connectionId.trim();
    if (typeof input.endpointClass === 'string' && input.endpointClass.trim()) model.endpointClass = input.endpointClass.trim();
    return model;
}
...
const SETTINGS_SECRET_REF = /^provider_keys\.([a-z0-9][a-z0-9-]*)$/;
const SERVER_SECRET_REF = /^server-secrets\.([a-z0-9][a-z0-9-]*)(?::[A-Za-z0-9._-]+)?$/;
const CUSTOM_CREDENTIAL_REF = /^custom:([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/;

/** Custom credential references are opaque handles, never raw key values. */
export function isSafeCustomCredentialRef(credentialRef, connectionId) {
    if (credentialRef === null) return true;
    if (typeof credentialRef !== 'string' || typeof connectionId !== 'string') return false;
    const credentialId = CUSTOM_CREDENTIAL_REF.exec(credentialRef)?.[1];
    return Boolean(credentialId && connectionId === `connection:${credentialId}`);
}

/** Secret references are handles into known settings/server stores, never values. */
export function isSafeSecretRef(secretRef, kind, provider = {}) {
    if (typeof secretRef !== 'string' || !secretRef.trim()) return false;
    const value = secretRef.trim();
    if (kind === 'server-adapter') return SERVER_SECRET_REF.exec(value)?.[1] === provider.id;
    if (kind !== 'browser-api-key') return false;
    const settingsMatch = SETTINGS_SECRET_REF.exec(value);
    if (settingsMatch?.[1] === provider.id) return true;
    return provider.id === 'linkapi' && value === 'linkapi_key';
}
...
export function normalizeProviderDefinition(input = {}) {
    ...
    return {
        ...
        ...(typeof input.unavailableReason === 'string' && input.unavailableReason.trim() ? { unavailableReason: input.unavailableReason.trim().slice(0, 240) } : {}),
...
export function normalizeProviderConnection(input = {}, provider = {}) {
    ...
    if (isSafeSecretRef(input.secretRef, kind, provider)) connection.secretRef = input.secretRef.trim();
...
export { CAPABILITY_STATES, EVIDENCE_SOURCES, POSTURES, CONNECTION_KINDS, ROUTE_EVIDENCE_STATES, ROUTE_EVIDENCE_SOURCES, ROUTE_PROTOCOLS };
