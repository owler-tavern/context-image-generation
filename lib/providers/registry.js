...
// These IDs are exposed by the installed SillyTavern Gemini host. They stay
// host-owned: a proxy catalog must independently advertise its exact IDs.
const HOST_GEMINI_FINAL_MODELS = [
    { id: 'gemini-3.1-flash-image', label: 'Nano Banana 2 🍌 (Flash, current host)', variant: 'flash2', imageSizeOptions: FLASH_2_IMAGE_SIZES, referenceImages: { maxCount: 4 }, supportsThinking: true, supportsGoogleSearch: true, transport: 'host-chat-image', status: 'existing' },
    { id: 'gemini-3-pro-image', label: 'Nano Banana Pro 🍌 (current host)', variant: 'pro', imageSizeOptions: PRO_IMAGE_SIZES, referenceImages: { maxCount: 14 }, transport: 'host-chat-image', status: 'existing' },
];
...
export const PROVIDERS = {
    makersuite: {
        ...
        models: [...GEMINI_MODELS, ...HOST_GEMINI_FINAL_MODELS],
    },
    linkapi: {
        id: 'linkapi',
        label: 'LinkAPI',
        ...
        ui: {
            ...
            modelDiscovery: { endpoint: '/models', responseFormat: 'openai-list' },
        ...
        discovery: { kind: 'openai-list', endpoint: '/models', transportId: 'openAiImages', retryable: true },
        retainUnresolvedCatalogModels: true,
        // https://docs.linkapi.ai/ documents this exact model/Images route.
        // Catalog presence alone does not establish optional image capabilities.
        authoritativeCatalogRoutes: {
            'gpt-image-2': {
                transportId: 'openAiImages', endpoint: 'https://linkapi.ai/v1',
                routeEvidence: {
                    state: 'verified', source: 'official-docs', observedAt: '2026-09-05T00:00:00.000Z',
        ...
        catalogModelRoutes: {
            'gemini-2.5-flash-image': { classification: 'accepted', modelId: 'gemini-2.5-flash-image' },
            'gemini-3.1-flash-image-preview': { classification: 'accepted', modelId: 'gemini-3.1-flash-image-preview' },
            'gemini-3-pro-image-preview': { classification: 'accepted', modelId: 'gemini-3-pro-image-preview' },
            'gpt-image-2-c': { classification: 'accepted', modelId: 'gpt-image-2-c' },
        },
    },
    tokenreply: {
        id: 'tokenreply',
        ...
        catalogModelRoutes: {
            'grok-imagine-image': { classification: 'accepted', modelId: 'grok-imagine-image' },
            'grok-imagine-image-quality': { classification: 'accepted', modelId: 'grok-imagine-image-quality' },
            'gemini-2.5-flash-image': { classification: 'unresolved', label: 'Nano Banana (Unverified)', note: 'Unverified: TokenReply has not supplied an evidenced Gemini image endpoint.' },
        },
    },
    openrouter: {
        id: 'openrouter',
...
const BUILT_IN_ROUTE_EVIDENCE = Object.freeze({
    openAiImages: Object.freeze({ state: 'verified', source: 'built-in', observedAt: '2026-08-31T00:00:00.000Z', protocol: 'openai-images', requestShapeRevision: 'openai-images-v1' }),
    sillyTavernGeminiProxy: Object.freeze({ state: 'verified', source: 'built-in', observedAt: '2026-08-31T00:00:00.000Z', protocol: 'gemini-compatible', requestShapeRevision: 'st-gemini-proxy-v1' }),
    'host-chat-image': Object.freeze({ state: 'verified', source: 'built-in', observedAt: '2026-08-31T00:00:00.000Z', protocol: 'gemini-compatible', requestShapeRevision: 'st-gemini-proxy-v1' }),
    'zai-native': Object.freeze({ state: 'verified', source: 'built-in', observedAt: '2026-08-31T00:00:00.000Z', protocol: 'zai-native', requestShapeRevision: 'zai-native-v1' }),
    'arliai-native': Object.freeze({ state: 'verified', source: 'built-in', observedAt: '2026-08-31T00:00:00.000Z', protocol: 'arliai-native', requestShapeRevision: 'arliai-native-v1' }),
});

for (const provider of Object.values(PROVIDERS)) {
    if (!Array.isArray(provider.models)) continue;
    provider.models = provider.models.map((model) => {
        const routeEvidence = model.routeEvidence || BUILT_IN_ROUTE_EVIDENCE[model.transport];
        return routeEvidence ? { ...model, routeEvidence } : model;
    });
}
...
export function getModelDefinition(providerId, modelId) {
    const provider = getProviderDefinition(providerId);
    return (provider?.models || []).find((candidate) => candidate.id === modelId);
}
...
export function resolveAdapterId(transportId) {
    return typeof transportId === 'string' ? ADAPTER_IDS[transportId] || null : null;
}
...
export function validateProviderRoute({ providerId, modelId, transportId, endpoint, modelDefinition, adapterRegistry } = {}) {
    ...
    const expectedAdapter = resolveAdapterId(declaredTransport);
