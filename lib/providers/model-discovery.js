...
import { connectionRevision, resolveCustomModelRoute, safeConnectionProjection, validateCustomConnection } from './custom-connections.js';
...
export function classifyDiscoveredModel(provider, item) {
    ...
    if (!declaration) return provider.retainUnresolvedCatalogModels === true
        ? { classification: 'unresolved', model: unresolvedCatalogModel(id, { note: 'Returned by the provider catalog. Image generation protocol is unknown; configure a custom connection with the model’s documented protocol to try it.' }) }
        : { classification: 'rejected', reason: 'no-declared-route' };
...
function safeSavedCustomModels(value, connection, existingEvidence) {
    ...
    const evidenceMatches = !connection.generationMethods && ['configured', 'verified'].includes(existingEvidence?.state)
        && existingEvidence?.revision === projection.revision && observedAt;
    const legacyRouteEvidence = evidenceMatches ? {
    ...
    return (Array.isArray(value) ? value : []).map((entry) => {
        ...
        if (!id || seen.has(id)) return null;
        seen.add(id);
        const route = resolveCustomModelRoute(connection, { id, transportId: entry?.transportId });
        if (!route) return null;
        const routeEvidence = connection.generationMethods
            ? { state: 'configured', source: 'user-configured-protocol', protocol: route.protocol, requestShapeRevision: route.protocol === 'gemini-compatible' ? 'st-gemini-proxy-v1' : 'openai-images-v1', revision: route.revision }
            : legacyRouteEvidence;
        return {
            id,
...
export async function discoverCustomConnectionModels({
    ...
    const requiredPreset = connection.catalogAuth || (connection.credentialRef ? (connection.protocol === 'gemini-compatible' ? 'gemini-api-key' : 'bearer') : 'none');
    ...
    try {
        ...
        const models = items.map((item) => {
            ...
            if (!id || seen.has(id)) return null;
            seen.add(id);
            const prior = savedModels.find((entry) => normalizeModelId(entry?.id) === id);
            const route = resolveCustomModelRoute(connection, { id, transportId: prior?.transportId || (connection.protocol === 'gemini-compatible' ? 'sillytavern-gemini-proxy' : 'openai-images') });
            if (!route) return null;
            return {
                id,
                label: id,
                ...
                routeEvidence: connection.generationMethods
                    ? { state: 'configured', source: 'user-configured-protocol', observedAt, protocol: route.protocol, requestShapeRevision: route.protocol === 'gemini-compatible' ? 'st-gemini-proxy-v1' : 'openai-images-v1', revision: route.revision }
                    : routeEvidence,
                posture: 'experimental',
            };
        }).filter(Boolean);
...
export function createModelDiscoveryCoordinator() {
    ...
    const start = (providerId, operation) => {
    ...
    return {
        refresh(providerId, options = {}) {
            return start(providerId, (signal) => discoverProviderModels({ ...options, providerId, signal }));
        },
        refreshCustom(providerId, options = {}) {
            return start(providerId, (signal) => discoverCustomConnectionModels({ ...options, signal }));
        },
        cancel(providerId) {
