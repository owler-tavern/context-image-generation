...
function isImageCapableModel(item) {
    ...
    return /(?:gpt[-_]?image|dall[-_]?e|flux|stable[-_]?diffusion|imagen|nano[-_]?banana|sdxl|image)/iu.test(String(item?.id || ''));
}
...
export function parsePollinationsImageModels(json) {
    ...
    return catalogItems(json).map((item) => {
        ...
        seen.add(id);
        return { id, label: String(item?.name || item?.label || id), transport: 'openAiImages', capabilities: {} };
    }).filter(Boolean);
}
...
export function parseNanoGptDetailedCatalog(json) {
    ...
    return catalogItems(json).map((item) => {
        ...
        if (!id || seen.has(id)) return null;
        seen.add(id);
        const inputModalities = (item?.input_modalities || item?.inputModalities || []).map((value) => String(value).toLowerCase());
        const outputModalities = (item?.output_modalities || item?.outputModalities || []).map((value) => String(value).toLowerCase());
        const capabilities = {};
        if (inputModalities.includes('image')) capabilities.referenceImages = { state: 'supported', source: 'official-docs', confidence: 'high' };
        const maxImages = Number(item?.max_images || item?.maxImages || 0);
        if (maxImages > 1) capabilities.multipleOutputs = { state: 'supported', source: 'official-docs', confidence: 'high' };
        if (outputModalities.includes('image') && !capabilities.referenceImages && !capabilities.multipleOutputs) {
            // Keep an explicitly declared image output as a note only. It does
            // not enable any optional request field.
