...
function isRetryableError(error) {
    return error?.name !== 'AbortError' && (!error?.status || error.status === 0);
}
...
export async function discoverProviderModels({
    ...
    if (discovery.kind === 'unsupported') {
        return {
            ...
            warning: { code: 'DISCOVERY_UNSUPPORTED', userMessage: discovery.reason || 'Model discovery is not available for this provider.' },
        };
    }
    ...
    if (!baseUrl || !discovery.endpoint) {
        return {
            models: [],
            evidence,
            warning: { code: 'DISCOVERY_MISCONFIGURED', userMessage: 'Model discovery is not configured for this provider.' },
        };
    }
    ...
    try {
        ...
        return {
            ...
            warning: {
                code: error?.status === 401 || error?.status === 403 ? 'DISCOVERY_AUTH_FAILED' : 'DISCOVERY_FAILED',
                userMessage: error?.message || 'Model discovery failed. Your current model list was kept.',
            },
        };
    }
}
...
export async function fetchProviderModels(options = {}) {
    ...
    if (result.warning) throw new Error(result.warning.userMessage);
...
export function createModelDiscoveryCoordinator() {
    ...
    return {
        refresh(providerId, options = {}) {
            if (active.has(providerId)) return active.get(providerId).promise;
