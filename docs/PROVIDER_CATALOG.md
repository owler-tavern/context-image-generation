...
| NanoGPT | Experimental | Verified `https://nano-gpt.com/v1/images/generations` transport plus a detailed `/api/v1/image-models?detailed=true` parser seam. | Explicit catalog metadata may describe references/multiple outputs; exact IDs alone remain unknown. No live generation recorded. |
...
| Z.AI | Future Server Adapter | Visible as unavailable because its native `quality`/`size` payload does not fit the validated OpenAI Images transport. | No browser runtime call is registered. Requires a separately implemented server/native adapter. |
...
| NovelAI, Stability AI, Naistera, ArliAI, Chutes | Future Server Adapter | Registry/catalog records only for provider-native binary, conflicting, or unverified browser routes. | Requires a narrow adapter and server/CORS/security evidence before implementation. |
