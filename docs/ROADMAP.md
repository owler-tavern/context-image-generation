...
The `v2` branch contains the provider foundation and deterministic implementations of the P1-P6 flows. At `410a6c6`, the expanded deterministic suite is green at **790/790**. This is **PROVEN deterministic** evidence for the listed contracts and production wiring; it does not by itself prove the promised player outcomes in a real host.
...
| P1 | Appearance truth, remembered looks, locks, outfits, and explainable continuity | A for chat canon, locks, outfits, multi-character/reference planning; B for avatar/description controls and reference explanations | **PARTIAL:** deterministic continuity/canon/outfit/reference contracts, current-chat identity isolation, Settings readiness, and final-plan receipts (`test/rp-appearance-*.test.mjs`, `test/rp-reference-*.test.mjs`, `test/rp-outfit-lock.test.mjs`); **PARTIAL live:** three single-character and two two-character attachments, but the benchmark quality scorecard is P1/failing | **NOT TESTED:** valid replacement two-character sequence after persona correction, scored identity retention, lock persistence, and live receipt comprehension |
